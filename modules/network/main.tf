...
// SG to allow all instance to instance traffic
resource "aws_security_group" "allow_instance_to_instance_pub" {
  name        = "lab-allow-all-instances"
  description = "Allow all traffic between public instances"
  vpc_id      = aws_vpc.lab-vpc.id
  ingress {
    from_port   = 0
    to_port     = 65535
